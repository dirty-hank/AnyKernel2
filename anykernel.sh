...
# shell variables
block=`find /dev/block/platform -path */f9824900.sdhci/by-name/boot`;
...
replace_string fstab.qcom "#/dev/block/zram0" "/dev/block/zram0" "#/dev/block/zram0"

replace_string init.mmi.rc "scheduler noop" "scheduler cfq" "scheduler noop"
...
case "$cmdtmp" in 
	*cpu_max_a53*) ;;
	*) echo "$cmdtmp cpu_max_a53=1536000 cpu_max_a57=2016000 boot_cpus=0-3" > $split_img/boot.img-cmdline;;
esac
